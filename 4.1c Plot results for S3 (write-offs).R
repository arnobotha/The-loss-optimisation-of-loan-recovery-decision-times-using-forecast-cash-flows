# =================== Graphing Script
# Plot results across forecasting techniques and parametrised/trained from various samples, procedure applied on s3 (write-offs)
...
#### RANDOM DEFAULTS : Weibull-truncation
it.name <- paste0("v2_5d(vi)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- data.table(Setup="a_s13-Weibull", dat.EL.par)
it.name <- paste0("v2_5e(ix)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="b_s23-Weibull", dat.EL.par))
it.name <- paste0("v2_5f(ix)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="c_s33-Weibull", dat.EL.par))

#### MARKOVIAN DEFAULTS
it.name <- paste0("v2_6a(iii)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="g_s13-Markov", dat.EL.par))
it.name <- paste0("v2_6b(iii)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="h_s23-Markov", dat.EL.par))
it.name <- paste0("v2_6c(iii)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="i_s33-Markov", dat.EL.par))
...
TotalBalance <- subset(dat.main, Measure == "CD" & Setup == "c_s33-Weibull")[order(Threshold),][1, Bal_Def]
...
plot.data4 <- subset(plot.data3, Setup %in% c("g_s13-Markov","h_s23-Markov"))
chosenFont <- "Times New Roman"

# create Legend text, with found minima in brackets
label.vec <- list(); i <- 1
label.vec[[i]] <- parse(text=paste0("Random~(italic(k)%~%Weibull)-italic(s)[13]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Random~(italic(k)%~%Weibull)-italic(s)[23]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Random~(italic(k)%~%Weibull)-italic(s)[33]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Markov-italic(s)[13]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Markov-italic(s)[23]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Markov-italic(s)[33]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
size.v <- c(0.6, 0.6, 0.9, 0.6, 0.6, 0.9)
ltype.v <- c("dotted", "dotted", "solid", "dotted", "dotted", "solid")
...
# zoom bounding box
xlim <- c(32,38); ylim <- c(0.198, 0.221)
...
plot.merged <- plot.full + annotation_custom(grob = ggplotGrob(plot.zoom), xmin = 25, xmax=110,
                                             ymin = use.ymin, ymax = use.ymax)
plot.merged

dpi <- 200
# note the 9-version graph was previously set to LossThreshv2
ggsave(plot.merged, file=paste0("LossThreshv3_CD-On_s3_rate", exp_version, ".png"),width=1200/dpi, height=1100/dpi,dpi=dpi)
