...
# - Forecasts trained from: S3
# - Optimisation applied on: S3
...
it.name <- paste0("MonteCarlo_EL",MC.size,"-v1_3a")
unpack.ffdf(paste0(it.name))
dat.main <- data.table(Setup="a_s33-Exp", dat.EL.MC)
it.name <- paste0("MonteCarlo_EL",MC.size,"-v1_3b")
unpack.ffdf(paste0(it.name))
dat.main <- rbind(dat.main,data.table(Setup="b_s33-Markov", dat.EL.MC))
...
TotalBalance <- subset(dat.main, Measure == "CD" & Setup == "a_s33-Exp" & Iteration == 1 & Threshold == 0)[1, Bal_Def]
...
label.vec <- list(); i <- 1
label.vec[[i]] <- parse(text=paste0(MC.size,"~Monte~Carlo~trials:~Random~(italic(k)%~%Exp)-italic(s)[33]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0(MC.size,"~Monte~Carlo~trials:~Markov-italic(s)[33]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
ltype.v <- c("solid", "solid")
shape.v <- c(1,16) # 3,4,8,
col.v <- (brewer.pal(n=8, name="Set1"))[c(4,7)]
...
# -- Zoomed plot 1
# zoom bounding box
xlim <- c(32.75, 38); ylim <- c(0.218, 0.2217)
# zoomed plot definition
plot.zoom <- 
  plot.full + coord_cartesian(xlim=xlim, ylim=ylim) + 
  geom_point(aes(x=Threshold-0.1,y=LossRate, color=Setup), size=0.1, position=position_jitterdodge(0.18), alpha=0.3) +
  geom_boxplot(aes(group=Threshold, x=Threshold-0.3), outlier.shape = NA, alpha=0.2, width=0.2, colour="grey30",
               data=subset(toplot, Setup == "a_s33-Exp" & Threshold %in% 30:40)) + 
...
# -- Zoomed plot 2
# zoom bounding box
xlim <- c(32.6, 38); ylim <- c(0.1975, 0.2053)
# zoomed plot definition
plot.zoom2 <- 
  plot.full + coord_cartesian(xlim=xlim, ylim=ylim) + 
  geom_point(aes(x=Threshold-0.475,y=LossRate, color=Setup), size=0.1, position=position_jitterdodge(0.18), alpha=0.5) +
  geom_boxplot(aes(group=Threshold, x=Threshold-0.3), outlier.shape = NA, alpha=0.2, width=0.2, colour="grey30",
               data=subset(toplot, Setup == "b_s33-Markov" & Threshold %in% 30:40)) + 
  geom_segment(aes(x=0,xend=Threshold, y=LossRate, yend=LossRate, color=Setup), linetype="dashed", data=plot.data3) + 
  geom_segment(aes(x=Threshold,xend=Threshold, y=min.y, yend=LossRate, color=Setup), linetype="dashed", data=plot.data3) +
  geom_errorbar(aes(ymin=Mean_LossRate_Lower, ymax=Mean_LossRate_Upper, x=Threshold), width=0.15, data=toplot.aggr, colour="grey30") + 
  theme(legend.position="none", axis.text.x = element_text(margin=unit(c(0,0,0,0),"mm"),size=9),
...
plot.merged <- plot.full + annotation_custom(grob = ggplotGrob(plot.zoom), xmin = 2, xmax=40,
...
plot.merged2 <- plot.merged + annotation_custom(grob = ggplotGrob(plot.zoom2), xmin = 41, xmax=79,
                                                ymin = use.ymin, ymax = use.ymax)
plot.merged2

dpi <- 200
# note the 9-version graph was previously set to LossThreshv2
ggsave(plot.merged2, file=paste0("LossThreshv3_CD-On_s3_rate_MC",MC.size,".png"),width=1200/dpi, height=1100/dpi,dpi=dpi)
