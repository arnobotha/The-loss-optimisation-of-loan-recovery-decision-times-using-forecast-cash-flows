# =================== Graphing Script
# Plot results across forecasting techniques and parametrised/trained from various samples, procedure applied on s1 (full sample)
...
it.name <- paste0("v2_5j(i)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- data.table(Setup="d_s11-Exp", dat.EL.par)
it.name <- paste0("v2_5k(i)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="e_s21-Exp", dat.EL.par))
it.name <- paste0("v2_5l(i)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="f_s31-Exp", dat.EL.par))

#### MARKOVIAN DEFAULTS
it.name <- paste0("v2_6a(i)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="g_s11-Markov", dat.EL.par))
it.name <- paste0("v2_6b(i)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="h_s21-Markov", dat.EL.par))
it.name <- paste0("v2_6c(i)", exp_version)
unpack.ffdf(paste0("EL",it.name))
dat.main <- rbind(dat.main,data.table(Setup="i_s31-Markov", dat.EL.par))
...
TotalBalance <- subset(dat.main, Measure == "CD" & Setup == "g_s11-Markov")[order(Threshold),][1, Bal_Def]
...
plot.data4 <- subset(plot.data3, Setup %in% c("g_s11-Markov", "h_s21-Markov"))
chosenFont <- "Times New Roman"

# -- create Legend text, with found minima in brackets
label.vec <- list(); i <- 1
label.vec[[i]] <- parse(text=paste0("Random~(italic(k)%~%Exp)-italic(s)[11]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Random~(italic(k)%~%Exp)-italic(s)[21]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Random~(italic(k)%~%Exp)-italic(s)[31]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Markov-italic(s)[11]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Markov-italic(s)[21]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0("Markov-italic(s)[31]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
size.v <- c(0.9, 0.6, 0.6, 0.9, 0.6, 0.6)
ltype.v <- c("solid", "dotted", "dotted", "solid", "dotted", "dotted")
...
plot.merged <- plot.full + annotation_custom(grob = ggplotGrob(plot.zoom), xmin = 5, xmax=90,
                                             ymin = use.ymin, ymax = use.ymax)
plot.merged

dpi <- 200
# note the 9-version graph was previously set to LossThreshv2
ggsave(plot.merged, file=paste0("LossThreshv3_CD-On_s1_rate", exp_version, ".png"),width=1200/dpi, height=1100/dpi,dpi=dpi)
