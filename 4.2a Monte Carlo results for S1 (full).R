...
# - Forecasts trained from: S1
# - Optimisation applied on: S1
...
it.name <- paste0("MonteCarlo_EL",MC.size,"-v1_1a")
unpack.ffdf(paste0(it.name))
dat.main <- data.table(Setup="a_s11-Exp", dat.EL.MC)
it.name <- paste0("MonteCarlo_EL",MC.size,"-v1_1b")
unpack.ffdf(paste0(it.name))
dat.main <- rbind(dat.main,data.table(Setup="b_s11-Markov", dat.EL.MC))
...
TotalBalance <- subset(dat.main, Measure == "CD" & Setup == "a_s11-Exp" & Iteration == 1 & Threshold == 0)[1, Bal_Def]
...
label.vec <- list(); i <- 1
label.vec[[i]] <- parse(text=paste0(MC.size,"~Monte~Carlo~trials:~Random~(italic(k)%~%Exp)-italic(s)[11]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
label.vec[[i]] <- parse(text=paste0(MC.size,"~Monte~Carlo~trials:~Markov-italic(s)[11]~~(italic(d)^ '*' ==", plot.data3[i,Threshold], ")")); i <- i + 1
ltype.v <- c("solid", "solid")
shape.v <- c(1,16) # 3,4,8,
col.v <- (brewer.pal(n=8, name="Set1"))[c(1,2)]
...
# -- Zoomed plot 1
# zoom bounding box
xlim <- c(3.75,6); ylim <- c(0.268, 0.269)
...
plot.zoom <- 
  ...
  geom_boxplot(aes(group=Threshold, x=Threshold-0.21), outlier.shape = NA, alpha=0.2, width=0.12, colour="grey30",
               data=subset(toplot, Setup == "a_s11-Exp" & Threshold %in% 2:7)) + 
...
# -- Zoomed plot 2
# zoom bounding box
xlim <- c(6.6,11); ylim <- c(0.107, 0.129)
# zoomed plot definition
plot.zoom2 <- 
  ...
  geom_boxplot(aes(group=Threshold, x=Threshold-0.3), outlier.shape = NA, alpha=0.2, width=0.2, colour="grey30",
               data=subset(toplot, Setup == "b_s11-Markov" & Threshold %in% 6:20)) + 
  geom_segment(aes(x=0,xend=Threshold, y=LossRate, yend=LossRate, color=Setup), linetype="dashed", data=plot.data3) + 
  geom_segment(aes(x=Threshold,xend=Threshold, y=min.y, yend=LossRate, color=Setup), linetype="dashed", data=plot.data3) +
  geom_errorbar(aes(ymin=Mean_LossRate_Lower, ymax=Mean_LossRate_Upper, x=Threshold), width=0.15, data=toplot.aggr, colour="grey30") + 
  theme(legend.position="none", axis.text.x = element_text(margin=unit(c(0,0,0,0),"mm"),size=9),
...
plot.merged <- plot.full + annotation_custom(grob = ggplotGrob(plot.zoom), xmin = 3, xmax=40,
...
plot.merged2 <- plot.merged + annotation_custom(grob = ggplotGrob(plot.zoom2), xmin = 20, xmax=57,
                                                ymin = use.ymin, ymax = use.ymax)
plot.merged2

dpi <- 200
# note the 9-version graph was previously set to LossThreshv2
ggsave(plot.merged2, file=paste0("LossThreshv3_CD-On_s1_rate_MC", MC.size,".png"),width=1200/dpi, height=1100/dpi,dpi=dpi)
